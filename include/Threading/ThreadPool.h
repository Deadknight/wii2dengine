...
public:
	...
	DEDOSAN_INLINE uint32 GetId() { return (uint32)thread_id; }
};
...
public:
	...
	void Suspend()
	{
		...
		LWP_SuspendThread(handle);
	}
	void Resume()
	{
		//pthread_cond_signal(&cond);
		LWP_ResumeThread(handle);
	}
	void Join()
	{
		//pthread_join(handle,NULL);
		LWP_JoinThread(handle,NULL);
...
public:
	...
	DEDOSAN_INLINE void Gobble() { _threadsEaten=(int32)m_freeThreads.size(); }

	// gets active thread count
	DEDOSAN_INLINE uint32 GetActiveThreadCount() { return (uint32)m_activeThreads.size(); }

	// gets free thread count
	DEDOSAN_INLINE uint32 GetFreeThreadCount() { return (uint32)m_freeThreads.size(); }
};

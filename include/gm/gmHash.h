...
public:
  ...
  public:
    ...
    GM_INLINEWII Iterator(const gmHash * a_hash)
    {
      m_hash = a_hash;
      m_slot = 0;
      m_elem = NULL;
    ...
    GM_INLINEWII void Dec(void) { GM_ASSERT(false); }
    GM_INLINEWII T * Resolve(void) { return m_elem; }
    GM_INLINEWII const T * Resolve(void) const { return m_elem; }
    GM_INLINEWII bool IsValid() const { return (m_elem != NULL); }
  ...
  GM_INLINEWII gmuint Count() const { return m_count; }
  GM_INLINEWII Iterator First() const { return Iterator(this); }
...
public:
  ...
  static GM_INLINEWII gmuint Hash(const char * a_key)
  {
    gmuint key = 0;
  ...
  static GM_INLINEWII int Compare(const char * a_keyA, const char * a_keyB)
  {
    return strcmp(a_keyA, a_keyB);
  }

  static GM_INLINEWII gmuint Hash(int a_key) 
  {
    return (gmuint) a_key;
  }

  static GM_INLINEWII int Compare(int a_keyA, int a_keyB)
  {
    return (a_keyA - a_keyB);
  }
  
  static GM_INLINEWII gmuint Hash(const void * a_key) 
  {
    return (gmuint) (((gmuint) a_key) / sizeof(double));
  }

  static GM_INLINEWII int Compare(const void * a_keyA, const void * a_keyB)
  {

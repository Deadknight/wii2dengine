...
public:
  ...
  GM_INLINEWII void SetColor(int a_color)               {m_color = (char)a_color;}
  GM_INLINEWII int GetColor()                           {return (int)m_color;}
  GM_INLINEWII gmGCObjBase* GetPrev() const             {return m_prev;}
  GM_INLINEWII void SetPrev(gmGCObjBase* a_prev)        {m_prev = a_prev;}
  GM_INLINEWII gmGCObjBase* GetNext() const             {return m_next;}
  GM_INLINEWII void SetNext(gmGCObjBase* a_next)        {m_next = a_next;}

  GM_INLINEWII char GetPersist()                        {return m_persist;}
  GM_INLINEWII void SetPersist(bool a_flag)             {m_persist = a_flag;}
...
public:
  ...
  GM_INLINEWII  bool AnyGrays(void)
  {
  ...
  GM_INLINEWII void GrayAWhite(gmGCObjBase* a_obj);
...
public:
  ...
  GM_INLINEWII void WriteBarrier(gmGCObjBase* a_lObj /*, gmGCObjBase* a_rObj*/);
  ...
  GM_INLINEWII void GetNextObject(gmGCObjBase* a_obj)   {m_colorSet.GrayAWhite(a_obj);}

  /// \brief Called on a new object being allocated
  GM_INLINEWII void AllocateObject(gmGCObjBase* a_obj)  {m_colorSet.Allocate(a_obj);}

  /// \brief Get the current shade color since it is flipped each cycle.
  GM_INLINEWII int GetCurShadeColor()                   {return (m_curShadeColor);}

  /// \brief Is the object colored gray or black?
  GM_INLINEWII bool IsShaded(gmGCObjBase* a_obj)        {return (a_obj->GetColor() == m_curShadeColor);}
  ...
  GM_INLINEWII gmGCTraceState& GetTraceState()          {return m_traceState;}

  /// \brief Set the amount of work to do per increment of collecting
  GM_INLINEWII void SetWorkPerIncrement(int a_workPerIncrement)     {m_workPerIncrement = a_workPerIncrement;}
  /// \brief Set the amount of objects to destruct per increment of collecting
  GM_INLINEWII void SetDestructPerIncrement(int a_destructPerIncrement)     {m_maxObjsToDestructPerIncrement = a_destructPerIncrement;}

  /// \brief Get the amount of work to do per increment of collecting
  GM_INLINEWII int GetWorkPerIncrement()                {return m_workPerIncrement;}
  /// \brief Get the amount of objects to destruct per increment of collecting
  GM_INLINEWII int GetDestructPerIncrement()            {return m_maxObjsToDestructPerIncrement;}

  /// \brief Set function to be called before flip when dead objects are reclaimed. 
  /// Optional, so pass NULL to disable.
  GM_INLINEWII void SetFlipCallback(GCFlipCallBack a_flipCallback)  {m_flipCallback = a_flipCallback;}

  /// \brief Is collector currently turned off?
  GM_INLINEWII bool IsOff()                             {return m_gcTurnedOff;}
  ...
  GM_INLINEWII gmMachine* GetVM()                       {return m_gmMachine;}
...
protected:
  ...
  GM_INLINEWII bool IsDoneTracing()                     {return m_doneTracing;}
  ...
  GM_INLINEWII void ToggleCurShadeColor()               {m_curShadeColor = !m_curShadeColor;}

...
public:
  ...
  GM_INLINEWII gmMemFixed(unsigned int a_elementSize, unsigned int a_growSize = 64);
  GM_INLINEWII ~gmMemFixed();

  /// \brief Alloc() an element
  GM_INLINEWII void* Alloc();
  
  /// \brief Free() an element
  GM_INLINEWII void Free(void* a_ptr);
  
  /// \brief Reset()
  GM_INLINEWII void Reset();
  
  /// \brief ResetAndFreeMemory()
  GM_INLINEWII void ResetAndFreeMemory();

  /// \brief Presize
  GM_INLINEWII void Presize(int a_kbytes)               { m_memChain.Presize(a_kbytes); }

  GM_INLINEWII unsigned int GetElementSize();

  /// \brief GetSystemMemUsed will return the number of bytes allocated by the system.
  GM_INLINEWII unsigned int GetSystemMemUsed() const { return m_memChain.GetSystemMemUsed(); }

#ifdef GM_DEBUG_BUILD
  /// \brief GetMemUsed()
  GM_INLINEWII unsigned int GetMemUsed() const { return m_memUsed; }

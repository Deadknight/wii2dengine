...
public:
  ...
  GM_INLINEWII Flags GetFlags() const { return (Flags) m_flags; }
  ...
  GM_INLINEWII void SetSwapEndianOnWrite(bool a_state)  { m_swapEndian = a_state; }
  GM_INLINEWII bool GetSwapEndianOnWrite() const        { return m_swapEndian; }

  /// \brief Explicitly set endian, will swap if necessary.
  GM_INLINEWII void SetEndianOnWrite(gmEndian a_endian)
  {
  ...
  GM_INLINEWII gmEndian GetEndianOnWrite() const
  {
    int isLittle = gmIsLittleEndian();
...
private:
  ...
  GM_INLINEWII void SwapEndian(gmuint8 &a_x) { }
  GM_INLINEWII void SwapEndian(gmint8 &a_x) { }
  GM_INLINEWII void SwapEndian(gmuint16 &a_x) { a_x = (gmuint16)((a_x << 8) | ((a_x >> 8) & 0xff)); }
  GM_INLINEWII void SwapEndian(gmint16 &a_x) { a_x = (gmint16)((a_x << 8) | ((a_x >> 8) & 0xff)); }
  GM_INLINEWII void SwapEndian(gmuint32 &a_x) { a_x = (a_x << 24) | ((a_x << 8) & 0x00ff0000) | ((a_x >> 8) & 0x0000ff00) | ((a_x >> 24) & 0x000000ff); }
  GM_INLINEWII void SwapEndian(gmint32 &a_x) { a_x = (a_x << 24) | ((a_x << 8) & 0x00ff0000) | ((a_x >> 8) & 0x0000ff00) | ((a_x >> 24) & 0x000000ff); }
  GM_INLINEWII void SwapEndian(gmfloat &a_x) { SwapEndian((gmuint32 &) a_x); }
};

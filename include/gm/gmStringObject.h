...
public:
  ...
  GM_INLINEWII const char * GetKey() const { return m_string; }

  virtual int GetType() const { return GM_STRING; }
  virtual void Destruct(gmMachine * a_machine);

  GM_INLINEWII operator const char *() const { return m_string; }
  GM_INLINEWII const char * GetString() const { return m_string; }
  GM_INLINEWII int GetLength() const { return m_length; }

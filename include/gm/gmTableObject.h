...
public:
  ...
  GM_INLINEWII void Set(gmMachine * a_machine, int a_index, const gmVariable &a_value)
  {
    Set(a_machine, gmVariable(GM_INT, (gmptr)a_index), a_value);
  }

  GM_INLINEWII int Count() const { return m_slotsUsed; }
  ...
  GM_INLINEWII gmTableNode * GetFirst(gmTableIterator& a_it) const
  {
    a_it = IT_FIRST;
  ...
  GM_INLINEWII bool IsNull(gmTableIterator a_it) const
  {
    if(a_it == IT_NULL)
    {
      return true;
...
private:
  ...
  GM_INLINEWII gmTableNode * GetAtHashPos(const gmVariable* a_key) const
  {

...
namespace luaUnit
{
	LUAFUNC int Unit_GetHealth(lua_State * L, T * ptr)
	{
		...
		PUSH_UINT32(L, ptr->GetUInt32Value(UNIT_FIELD_HEALTH));
		return 1;
	}
	LUAADD(Unit_GetHealth, Player);
	LUAADD(Unit_GetHealth, Creature);
	LUAADD(Unit_GetHealth, Unit);

	LUAFUNC int Unit_SetHealth(lua_State * L, T * ptr)
	{
		LUAFUNCTHISCHECKNULL
		uint32 value = CHECK_UINT32(L, 1);
		ptr->SetUInt32Value(UNIT_FIELD_HEALTH, value);
		return 1;
	}
	LUAADD(Unit_SetHealth, Player);
	LUAADD(Unit_SetHealth, Creature);
	LUAADD(Unit_SetHealth, Unit);

	LUAFUNC int Unit_GetHealthPercent(lua_State * L, T * ptr)
	{
		LUAFUNCTHISCHECKNULL
		PUSH_UINT32(L, ((ptr->GetUInt32Value(UNIT_FIELD_HEALTH) * 100) / ptr->GetUInt32Value(UNIT_FIELD_MAXHEALTH)));
		return 1;
	}
	LUAADD(Unit_GetHealthPercent, Player);
	LUAADD(Unit_GetHealthPercent, Creature);
	LUAADD(Unit_GetHealthPercent, Unit);
	
	LUAFUNC int Unit_GetHealthMax(lua_State * L, T * ptr)
	{
		LUAFUNCTHISCHECKNULL
		PUSH_UINT32(L, ptr->GetUInt32Value(UNIT_FIELD_MAXHEALTH));
		return 1;
	}
	LUAADD(Unit_GetHealthMax, Player);
	LUAADD(Unit_GetHealthMax, Creature);
	LUAADD(Unit_GetHealthMax, Unit);

	LUAFUNC int Unit_SetHealthMax(lua_State * L, T * ptr)
	{
		LUAFUNCTHISCHECKNULL
		uint32 value = CHECK_UINT32(L, 1);
		ptr->SetUInt32Value(UNIT_FIELD_MAXHEALTH, value);
		return 1;		
	}
	LUAADD(Unit_SetHealthMax, Player);
	LUAADD(Unit_SetHealthMax, Creature);
	LUAADD(Unit_SetHealthMax, Unit);

	LUAFUNC int Unit_GetHealthBase(lua_State * L, T * ptr)
	{
		LUAFUNCTHISCHECKNULL
		PUSH_UINT32(L, ptr->GetUInt32Value(UNIT_FIELD_BASE_HEALTH));
		return 1;
	}
	LUAADD(Unit_GetHealthBase, Player);
	LUAADD(Unit_GetHealthBase, Creature);
	LUAADD(Unit_GetHealthBase, Unit);

	LUAFUNC int Unit_SetHealthBase(lua_State * L, T * ptr)
	{
		LUAFUNCTHISCHECKNULL
		uint32 value = CHECK_UINT32(L, 1);
		ptr->SetUInt32Value(UNIT_FIELD_BASE_HEALTH, value);
		return 1;
	}
	LUAADD(Unit_SetHealthBase, Player);
	LUAADD(Unit_SetHealthBase, Creature);
	LUAADD(Unit_SetHealthBase, Unit);

	LUAFUNC int Unit_GetMana(lua_State * L, T * ptr)
	{
		LUAFUNCTHISCHECKNULL
		PUSH_UINT32(L, ptr->GetUInt32Value(UNIT_FIELD_MANA));
		return 1;
	}
	LUAADD(Unit_GetMana, Player);
	LUAADD(Unit_GetMana, Creature);
	LUAADD(Unit_GetMana, Unit);

	LUAFUNC int Unit_SetMana(lua_State * L, T * ptr)
	{
		LUAFUNCTHISCHECKNULL
		uint32 value = CHECK_UINT32(L, 1);
		ptr->SetUInt32Value(UNIT_FIELD_MANA, value);
		return 1;
	}
	LUAADD(Unit_SetMana, Player);
	LUAADD(Unit_SetMana, Creature);
	LUAADD(Unit_SetMana, Unit);

	LUAFUNC int Unit_GetManaPercent(lua_State * L, T * ptr)
	{
		LUAFUNCTHISCHECKNULL
		PUSH_UINT32(L, ((ptr->GetUInt32Value(UNIT_FIELD_MANA) * 100) / ptr->GetUInt32Value(UNIT_FIELD_MAXMANA)));
		return 1;
	}
	LUAADD(Unit_GetManaPercent, Player);
	LUAADD(Unit_GetManaPercent, Creature);
	LUAADD(Unit_GetManaPercent, Unit);

	LUAFUNC int Unit_GetManaMax(lua_State * L, T * ptr)
	{
		LUAFUNCTHISCHECKNULL
		PUSH_UINT32(L, ptr->GetUInt32Value(UNIT_FIELD_MAXMANA));
		return 1;
	}
	LUAADD(Unit_GetManaMax, Player);
	LUAADD(Unit_GetManaMax, Creature);
	LUAADD(Unit_GetManaMax, Unit);

	LUAFUNC int Unit_SetManaMax(lua_State * L, T * ptr)
	{
		LUAFUNCTHISCHECKNULL
		uint32 value = CHECK_UINT32(L, 1);
		ptr->SetUInt32Value(UNIT_FIELD_MAXMANA, value);
		return 1;
	}
	LUAADD(Unit_SetManaMax, Player);
	LUAADD(Unit_SetManaMax, Creature);
	LUAADD(Unit_SetManaMax, Unit);

	LUAFUNC int Unit_GetManaBase(lua_State * L, T * ptr)
	{
		LUAFUNCTHISCHECKNULL
		PUSH_UINT32(L, ptr->GetUInt32Value(UNIT_FIELD_BASE_MANA));
		return 1;
	}
	LUAADD(Unit_GetManaBase, Player);
	LUAADD(Unit_GetManaBase, Creature);
	LUAADD(Unit_GetManaBase, Unit);

	LUAFUNC int Unit_SetManaBase(lua_State * L, T * ptr)
	{
		LUAFUNCTHISCHECKNULL
		uint32 value = CHECK_UINT32(L, 1);
		ptr->SetUInt32Value(UNIT_FIELD_BASE_MANA, value);
		return 1;
	}
	LUAADD(Unit_SetManaBase, Player);
	LUAADD(Unit_SetManaBase, Creature);
	LUAADD(Unit_SetManaBase, Unit);

	LUAFUNC int Unit_AddWeapon(lua_State * L , T * ptr)
	{
		LUAFUNCTHISCHECKNULL
		const char *name = CHECK_STRING(L, 1);
		int32 id = CHECK_INT32(L, 2);

		ptr->AddWeapon(name, id);
		return 1;
	}
	LUAADD(Unit_AddWeapon, Player);
	LUAADD(Unit_AddWeapon, Creature);
	LUAADD(Unit_AddWeapon, Unit);

	LUAFUNC int Unit_SetSelectedWeapon(lua_State * L, T * ptr)
	{
		LUAFUNCTHISCHECKNULL
		uint32 value = CHECK_UINT32(L, 1);
		ptr->SetSelectedWeapon(value);
		return 1;
	}
	LUAADD(Unit_SetSelectedWeapon, Player);
	LUAADD(Unit_SetSelectedWeapon, Creature);
	LUAADD(Unit_SetSelectedWeapon, Unit);
	...
	LUAADD(Unit_ChangeSprite, Player);
	LUAADD(Unit_ChangeSprite, Creature);
	LUAADD(Unit_ChangeSprite, Unit);
...
//		uint32 icon = CHECK_ULONG(L, 3);
//		uint32 flags = CHECK_ULONG(L, 4);
//		uint32 data = CHECK_ULONG(L, 5);
//		const char *name = luaL_checkstring(L, 6);
//		if(!posX || !posY || !icon) 
//		{
...
//			return 0;
//		}
//		TO_PLAYER(ptr)->Gossip_SendPOI(posX,posY,icon,flags,data,name);
//		return 0;
//	}
//
...
//		if(!ptr || !message)
//			return 0;
//		if( strncmp(message, "health", 7)==0 )
//			ptr->SetPowerType(POWER_TYPE_HEALTH);
//		else if( strncmp(message, "mana", 5)==0 )
//			ptr->SetPowerType(POWER_TYPE_MANA);
//		else if( strncmp(message, "rage", 5)==0 )
//			ptr->SetPowerType(POWER_TYPE_RAGE);
//		else if( strncmp(message, "focus", 6)==0 )
//			ptr->SetPowerType(POWER_TYPE_FOCUS);
//		else if ( strncmp(message, "energy", 7)==0 )
...
//			{
//			//hype: POWER_TYPE_HEALTH is -2, but power type is stored as uint
...
//	{
//		//uint32 spellid = CHECK_ULONG(L,1);
//		//What is the point of this method...
//		//if(ptr)
//			//ArcLuna<SpellEntry>::push(L,ptr->GetAIInterface()->getSpellEntry(spellid));
//		lua_pushnil(L);
//		return 1;
//	}
//
...
//	{
//		//This method sends an invalid AI_Spell struct to SetNextSpell. You can't simply cast a SpellEntry to an AI_Spell without some values being corrupted.
//		/*
//		SpellEntry * sp = dbcSpell.LookupEntryForced(CHECK_ULONG(L,1));
//		if(ptr && sp)
...
//		Player * plr = ((Player*)ptr);
//		plr->GetAchievementMgr().GMCompleteAchievement(NULL, achievementID);
//		return 1;
//	}
//
...
//		CHECK_TYPEID(TYPEID_PLAYER);
//		int32 achievementID = luaL_checkint(L,1);
//		((Player*)ptr)->GetAchievementMgr().GMResetAchievement(achievementID);
//		return 1;
//	}
//
...
//		lua_pushboolean(L, ((Player*)ptr)->GetAchievementMgr().HasCompleted(achievementID) ? 1 : 0);
//		return 1;
//	}*/
//
